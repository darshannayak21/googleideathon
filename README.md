...
Response + mood + stress data stored in user-isolated Firestore documents
        |
        v
Data encrypted at rest using Cloud KMS AES-256
```
...
### AI Insights Hub
An analytics dashboard that aggregates your emotional data locally and renders a live Radar Chart using Recharts. When triggered, the backend sends only your concise session summaries (not raw chat logs) to Gemini to synthesize high-level growth advice and identify recurring emotional patterns.

### Automatic Session Summarization
At the end of any session, Gemini generates a structured JSON summary containing a brief overview, dominant mood, and up to three topic tags. These summaries power the Insights Hub analytics.

### Real-time Mood and Stress Tracking
Every Gemini response includes a structured JSON block with a single-word mood label and a 1-10 stress score. This data is persisted alongside every message and visualized in the Insights Hub.

### Cloud KMS Encryption
Sensitive data is encrypted at rest using Google Cloud KMS with AES-256-GCM symmetric encryption. The encryption and decryption operations happen entirely server-side through the KMS API.
...
A dynamically resizing chat input that grows with your message, integrated persona switching via a drop-up menu, and voice-to-text input via the Web Speech API.
...
│   ├── gemini_client.py        # Gemini API wrapper (chat, summarize, embed, trends)
│   ├── secret_manager.py       # Secret Manager key retrieval
│   ├── models.py               # Pydantic request/response models
│   ├── routers/
│   │   ├── chat.py             # POST /api/chat
│   │   ├── sessions.py         # GET /api/sessions
│   │   ├── summarize.py        # POST /api/summarize/{session_id}
│   │   ├── insights.py         # POST /api/insights/synthesize-trends
