...
@router.post("")
async def create_new_task(text: str = Body(..., embed=True), uid: str = Depends(get_current_user)):
    """Create a new task."""
    try:
...
async def patch_task(task_id: str, updates: Dict[str, Any] = Body(...), uid: str = Depends(get_current_user)):
...
async def remove_task(task_id: str, uid: str = Depends(get_current_user)):
    """Delete a task."""
    try:
